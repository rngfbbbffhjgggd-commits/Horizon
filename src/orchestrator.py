...
_ARTICLE_TITLE_RE = re.compile(
    ...
    # treated as articles here. Everything subtler is left to the scoring prompt's
    # STEP 1 — a dedicated classifier pass was tried and rejected, see step 6.3.
...
class HorizonOrchestrator:
    ...
    async def run(self, force_hours: int = None, since_str: str = None) -> None:
        ...
        try:
            ...
            #
            #     NOTE 2026-09-25: a dedicated NEWS-vs-ARTICLE pass was written
            #     here to replace keyword rules and then REMOVED before shipping —
            #     measured on the 2026-09-25 digest, glm-4-flash labelled 15 of
            #     25 items ARTICLE (3 consecutive runs; 12 of 25 on titles only),
            #     including plain news such as the DRC Ebola outbreak, a gas
            #     futures jump and a Zelensky warning. A 6-item focused batch
            #     scored 6/6, so the failure is scale-dependent and therefore
            #     untrustworthy. Article-shaped items stay handled by
            #     _ARTICLE_TITLE_RE plus the scoring prompt's STEP 1.
